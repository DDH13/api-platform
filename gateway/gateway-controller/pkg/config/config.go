...
type CollectorConfig struct {
	...
	// e.g. health-check probes. Consumed by the policy-engine.
	...
	// transport that ships collected data. It is part of the collector.
...
type AnalyticsConfig struct {
	...
	// [collector].grpc_event_server; when set here it is migrated onto the
...
// and enforces the collector prerequisite: analytics (a consumer) cannot be enabled
// unless the collector that feeds it is enabled. It also validates the ALS transport
// tuning when the collector is enabled.
func (c *Config) validateCollectorConfig() error {
	...
	if c.Analytics.Enabled && !c.Collector.Enabled {
		return fmt.Errorf("analytics.enabled requires collector.enabled to be true (the collector provides the data analytics consumes)")
	}
	if c.Collector.MaxPayloadSize < 0 {
		return fmt.Errorf("collector.max_payload_size must be >= 0, got %d", c.Collector.MaxPayloadSize)
	}
...
func (c *Config) migrateDeprecatedAnalyticsTransport() {
	...
	if c.Analytics.GRPCEventServerCfg != def {
		slog.Warn("analytics.grpc_event_server is deprecated; use collector.grpc_event_server instead")
...
func validateGRPCEventServerConfig(cfg GRPCEventServerConfig) error {
	...
	case "tcp":
		...
		if cfg.Port <= 0 || cfg.Port > 65535 {
			return fmt.Errorf("collector.grpc_event_server.port must be between 1 and 65535 when mode is tcp, got %d", cfg.Port)
		}
	default:
		return fmt.Errorf("collector.grpc_event_server.mode must be 'uds' or 'tcp', got: %s", cfg.Mode)
	}
	...
	if cfg.ServerPort <= 0 || cfg.ServerPort > 65535 {
		return fmt.Errorf("collector.grpc_event_server.server_port must be between 1 and 65535, got %d", cfg.ServerPort)
	}
	return nil
}
