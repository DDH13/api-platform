...
// TestValidate_CollectorPrerequisite verifies that consumers (analytics, traffic
// logging) require the collector to be enabled.
func TestValidate_CollectorPrerequisite(t *testing.T) {
	...
	t.Run("analytics enabled without collector errors", func(t *testing.T) {
		cfg := validConfig()
		...
		err := cfg.Validate()
		require.Error(t, err)
		assert.Contains(t, err.Error(), "collector.enabled")
	})

	t.Run("traffic logging enabled without collector errors", func(t *testing.T) {
		cfg := validConfig()
		cfg.Collector.Enabled = false
